...
class PokecordTeller3(autoaction.AutoAction):

	async def run(self, message, client):
		#print("ran")
		pokecordid = int(os.environ.get("POKECORD_ID"))
		...
		if not testing: # testing is a bit hard, cus pokecord only spawns pokemon when you dont want it to
			...
			if not (message.author.id == pokecordid) and not testing: # if message is from pokecord
				return False
			if not (len(message.embeds) > 0) and not testing: # if message has embeds
				return False
			print (message.embeds[0].title) # actually really useful, and hella cool, tells you when users are blacklisted, when people catch stuff, etc
			if not (message.embeds[0].title.startswith("‌‌A wild pokémon has аppeаred!")): # ok we actually only want to respond to a wild pokemon stuff so
				return False
		if not testing:
