...
#{message.created_at}''')
			...
			embed.set_footer(text = f'''v2.compact|messageid,authorid,channelid,guildid,senttimeutc|{message.id},{message.author.id},{message.channel.id},DMChannel,{message.created_at}''') # v2.compact 2020-05-18
		else: # not dmchannel
#			embed.set_footer(text = f'''v2
#messageid,authorid,channelid,guildid,senttimeutc
#{message.id}
...
#{message.created_at}''') # v2 messageid,authorid,channelid,guildid,senttimeutc
			embed.set_footer(text = f'''v2.compact|messageid,authorid,channelid,guildid,senttimeutc|{message.id},{message.author.id},{message.channel.id},{message.guild.id},{message.created_at}''') # v2.compact 2020-05-18
		if len(message.attachments) > 0: # 2020-02-28 now adds support for recording images
			...
			imagemessage = await client.get_channel(int(os.environ.get("IMGLOGCHANNELID"))).send(file = discord.File(io.BytesIO(attachmentfile), filename = attachment.filename))
