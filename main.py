...
from chattriggers import replywithdot, kkk, nouk, hoesmad, guildcreationdate, meaning, translate, translateft, \
    anonymessage, changenickname, getprofilepicture, taguser, killbot, say, purgechannel, mee6levelfetcher, \
...
ctriggers.append(
    chattriggershelpcommand.ChatTriggersHelpCommand("Chat Triggers Help Command (,help)", [",help", "!help"],
                                                    ctriggers))
...
ctriggers.append(hoesmad.HoesMad("Hoes mad copypastae", ["hoes mad", "https://www.youtube.com/watch?v=J6oTIjvw_-8",
                                                         "https://tenor.com/view/hoes-mad-famous-dex-dance-gif-14199119"]))
...
ctriggers.append(taguser.TagUser("Tag User (,taguser [@member] [number of times to tag])",
                                 [",taguser ", ",pinguser ", ",tag ", ",ping "]))
...
ctriggers.append(purgeuntilmessage.PurgeUntilMessage("Purge Until Message (,purgeuntilmessage", [",purgeuntilmessage "],
                                                     dispatchTyping=False))
ctriggers.append(getinvitesofallguilds.GetInvitesOfAllGuilds("Get Invites of all guilds (,getinvitesofallguilds)",
                                                             [",getinvitesofallguilds"]))
ctriggers.append(addcolour.AddColour("Add Colour Role (,addcolour [colour code] [colour name]",
                                     [",addcolourrole ", ",addcolour ", ",ac "]))
ctriggers.append(
    getcolour.GetColour("Get Colour Role (,getcolour [colour role name]", [",getcolourrole ", ",getcolour ", ",gc "]))
ctriggers.append(
...
ctriggers.append(
    restorechannel.RestoreChannel("Restores Channels (,restorechannel [channel name]", [",restorechannel "]))
...
autoactions.append(chatresponse.ChatResponse(
    ...
    async def on_ready(self):  # on ready
        ...
        for i in startuptasks:  # startup tasks, new module which deprecated the following (commented out) code
            ...
            self.loop.create_task(i.run(
                client))  # startup tasks now run simultaneously, as startuptasks double as background tasks, this allows multiple tasks to run at once
