...
class AutoLimboChannel(onmemberjointask.OnMemberJoinTask):
	...
	async def run(self, member: discord.Member, client: discord.Client):
		...
		lchannel = await limbocategory.create_text_channel(member.id, overwrites = overwrites, position = 0, topic = f"Verification Channel for {str(member)} ({member.id})", slowmode_delay = slowmodetime, reason = f"Verification Channel for {str(member)} ({member.id})")
	...
	async def greet(self, member, channel): # to let the new user know what the hell is going on
		...
		await channel.send(f"{member.mention}, Welcome to {member.guild.name}. Please ask for someone to give you a role.") # 2020-05-06, the previous message just had users sitting around for a few minutes and then leaving
	
	# new addition

	async def findlimbocategory(self, member: discord.Member, client: discord.Client): # i dont want to overcomplicate the main function
