				...
				try:
					...
					for i in cservs:
						...
						if i["name"] in servers:
							...
							if not i["status"] == "ONLINE": # prob not needed as only online servers are shown in the serverlist anyways
								continue
							...
							if citeration - lastonlineiteration[servindex] != 1:
								await notifchannel.send(f"<@{os.environ.get('OWNER_ID')}>, {servname}.minehut.gg just started!")
							...
							await notifchannel.send(f"{servname}.minehut.gg is currently online! Online for: {ptime // 3600} hours, {ptime % 3600 // 60} minutes, {ptime % 60 // 1} seconds.\n{len(players)} online players: {', '.join(players)}")
